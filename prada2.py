...
reference_fasta = os.path.join(base_dir, 'GRCh38.d1.vd1.fa')
annotation_gtf = os.path.join(base_dir, 'gencode.v22.annotation.gtf')
...
parser.add_argument('--runThread', default=12, help='decide the number of thread used by STAR, default is 12')
...
if os.path.exists(case_dir):
    ...
    if not os.path.exists(genome_index_dir):
        ...
        Process = Popen(['bash ' + base_dir + '/check_index.sh %s %s %s %s' % (
    ...
    Process = Popen(['bash ' + base_dir + '/concise_mode_v1.0.sh %s %s %s %s %s %s %s %s %s %s' % (read1, read2,
    ...
    if Process.returncode != 0:
        ...
        check_dir(genome_index_dir)
        Process = Popen(['bash ' + base_dir + '/check_index.sh %s %s %s %s' % (genome_index_dir, reference_fasta, annotation_gtf, 8)],
                        shell=True)
        print Process.communicate()

        print 'redo alignment'
        Process = Popen(['bash ' + base_dir + '/concise_mode_v1.0.sh %s %s %s %s %s %s %s %s %s %s' % (
